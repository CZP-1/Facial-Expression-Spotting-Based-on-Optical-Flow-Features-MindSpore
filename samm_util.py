...
def crop_picture(img_rd,size):
    ...
    width_eye=int((right[0,0]-left[0,0])/2) #左右眼角宽度的一半
    center=[int((right[0,0]+left[0,0])/2),int((right[0,1]+left[0,1])/2)]

    cv2.rectangle(img_rd, (center[0] - int(4.5 * width_eye), center[1] - int(3.5 * width_eye)), (center[0] + int(4.5 * width_eye), center[1] + int(5.5 * width_eye)),
                  (0, 0, 255), 2)

    a=(center[1] - int(3.5 * width_eye))
    b=center[1] +int(5.5 * width_eye)
...
def fenxi(flow_total,imf_sum1,yuzhi1,yuzhi2): #使用寻找峰的方法 分解前的信号 分解后的信号 1.3 1.5
    flow_total = np.array(flow_total)
...
def process(flow1_total,yuzhi1,yuzhi2,position,xuhao,k,a,totalflow):
    ...
    filter_threshold=5
    flow1_total_edm1 = temporal_ideal_filter(flow1_total[a:-a], 1, filter_threshold, 30)  # 滤波
    ...
    # flow1_total_fenxi = fenxi(flow1_total_edm2,imf_sum1,yuzhi1,yuzhi2)
    ...
    while(k<len(fileList)):
        ...
        for i in fileList:
            ...
            if (k  == end):
                ...
                # print(k)
                a=1
                totalflow = process(flow1_total,1.4,1.65,"left_eye",0,k,a,totalflow)
                totalflow = process(flow3_total,1.4,1.65,"right_eye",0,k,a,totalflow)
                totalflow =process (flow4_total,1.4,1.5,"mouth",0,k,a,totalflow)
                totalflow = process(flow5_total1, 1.4, 1.5, "nose", 1, k, a, totalflow)
                totalflow = process(flow5_total2, 1.4, 1.5, "nose", 2, k, a, totalflow)
